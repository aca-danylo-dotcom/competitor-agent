...
class Settings(BaseSettings):
    ...
    # Обход сайта конкурента. Агент не угадывает, где лежит прайс: он берёт карту
    # сайта, проходит страницы и сам смотрит, на каких есть цены. Пределы нужны,
    # чтобы большой сайт не занял весь прогон — но заданы с запасом.
    crawl_page_limit: int = 80
    crawl_time_budget: float = 150.0
    # Сколько страниц с ценами разбирать моделью: прайс редко размазан шире.
    price_pages_limit: int = 6
